...
function buildPrompt(cardName, issuerName, summary) {
  return `Search the web right now: has "${issuerName}" recently made this specific change to its "${cardName}" credit card - "${summary}"?
...
If you find genuine independent coverage of this specific change, summarize what you found (including any date mentioned). If you find nothing, or only old/unrelated pages, say clearly that you found no independent corroboration.`;
}
...
const NO_CORROBORATION_RE =
  /\b(could not find|couldn't find|did not find|didn't find|no evidence|no reports?|no recent news|no information|could not confirm|couldn't confirm|unable to (find|confirm)|no results?|did not contain any information)\b/i;
 ...
 * carries at least one real citation AND doesn't itself say it found
 * nothing (see textIndicatesNoCorroboration - citations can be present
 * even on a "found nothing" answer), or null otherwise - including on a
 * missing key, a hit MAX_CALLS_PER_RUN cap, or any call/parse error. Never
 * throws - same "boost confidence when found, never block reporting"
 * contract as groundChangeInReddit.
 */
export async function groundChangeInGoogleSearch({ cardName, issuerName, summary, officialUrl }) {
  ...
  try {
    ...
    if (citations.length === 0 || textIndicatesNoCorroboration(answerText)) {
      return null;
    }
